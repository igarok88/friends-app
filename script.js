(async function () {
  ...
  let allUsers = await getUsers(ALL_USERS_LINK);

  if (!allUsers) {
    const result = confirm(
      "Sometimes the API from https://randomuser.me/ is unavailable. It looks like that's the case right now. Load the local database instead?"
    );
    if (result) {
      allUsers = await getUsers(LOCAL_DB);
    }
  }
